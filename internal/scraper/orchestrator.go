	...
	if concurrency > 1 && len(courses) > 1 {
		if dominantRepoID, ok := selectDominantCourse(courses, sizeHints); ok {
			...
			if len(dominant) == 1 {
				...
				dominantFiles := collectCourseFilesConcurrently(ctx, dominant, 1, collectFn, s.mergeDownloadCandidates)
				restFiles := collectCourseFilesConcurrently(ctx, rest, concurrency, collectFn, s.mergeDownloadCandidates)
				remoteFiles = append(dominantFiles, restFiles...)
			}
		}
	}
	if remoteFiles == nil {
		remoteFiles = collectCourseFilesConcurrently(ctx, courses, concurrency, collectFn, s.mergeDownloadCandidates)
	}
